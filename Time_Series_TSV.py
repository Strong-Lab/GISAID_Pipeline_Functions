...
def clean_metadata(metadata_with_variants,variant_events,metadata_variants_out=None):
    ...
    if metadata_variants_out:
        #For output only, print the 'variants' column as a comma-separated string rather than a list
        def list_to_string(lst):
            delim=","
            return delim.join(lst)
        standard_date_out=standard_date.copy(deep=True)
        standard_date_out=standard_date_out["Variants"].apply(lambda x:list_to_string(x))
        print("Writing cleaned metadata with variants to {}.".format(metadata_variants_out))
        standard_date_out.to_csv(metadata_variants_out,sep="\t")
    return standard_date
